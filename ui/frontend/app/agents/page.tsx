...
function ClassPaletteCard({ cls, label, desc, onAdd, onDragStart, onDragEnd }: {
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      onClick={onAdd}
      className="w-full flex items-center gap-2 p-2 rounded-xl border border-gray-800 hover:border-gray-600 bg-gray-900 hover:bg-gray-800 transition-all text-left group cursor-grab active:cursor-grabbing select-none"
    >
      ...
      <span className="text-gray-700 text-[8px] shrink-0">⠿</span>
    </div>
  );
}
...
function StepCard({
  ...
  isDragOver: boolean; isAnyDragActive: boolean;
  ...
  return (
    <div className="flex items-start shrink-0">
      ...
      >
        ...
        {/* ── Agent card body ── */}
        <div className={cn(
          "relative rounded-2xl border-2 bg-gray-900/60 w-48 overflow-hidden transition-all",
          isDragOver
            ? "border-teal-500 shadow-xl shadow-teal-900/50 scale-[1.03]"
            : isAnyDragActive
              ? "border-teal-800 border-dashed"
              : agentSel ? cn(meta.borderColor, "shadow-xl shadow-black/40") : "border-gray-800",
        )}>
          {/* Drop-to-add overlay */}
          {isDragOver && (
            <div className="absolute inset-0 bg-teal-900/30 flex flex-col items-center justify-center gap-1 pointer-events-none z-10 rounded-2xl">
              <Plus className="w-5 h-5 text-teal-300" />
              <span className="text-[10px] font-semibold text-teal-200">Drop to add</span>
            </div>
          )}
          {!compat && (
...
function AgentSettingsPanel({ step, allAgents, prevStepClass, onChangeStep, onSaveAgent, onClose }: {
  ...
  // inputs are shown on the canvas; only use keys for prompt variable hints
  const varKeys = (agent?.inputs ?? []).filter(i => i.key);
...
export default function AgentsPage() {
  ...
  // Set output format on a step by dropping an output_fmt from the left panel
  async function setOutputFmtOnStep(stepIdx: number, fmt: string) {
    const step  = pipelineForm.steps[stepIdx];
    const agent = allAgents.find(a => a.id === step.agent_id);
    if (!agent) return;
    await saveNodeAgent(agent.id, { ...agent, output_format: fmt });
  }
  ...
  return (
    <div className="min-h-[calc(100vh-5.25rem)] flex flex-col -m-6">
      ...
      <div className="flex-1 min-h-0 flex bg-gray-950">
        ...
        <div className="w-44 shrink-0 border-r border-gray-800 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            ...
            <div className="space-y-1.5 mb-3">
              {CLASS_TYPES.map(t => (
                <ClassPaletteCard
                  key={t.cls} cls={t.cls} label={t.label} desc={t.desc}
                  onAdd={() => addStep(t.cls)}
                  onDragStart={e => {
                    e.dataTransfer.setData("agent_cls", t.cls);
                    e.dataTransfer.effectAllowed = "copy";
                    setIsDragging(true);
                  }}
                  onDragEnd={() => { setIsDragging(false); setDragOverStep(null); }}
                />
              ))}
            </div>

            {/* Input sources */}
            <p className="text-[9px] text-gray-600 uppercase tracking-wider font-semibold px-0.5 mb-1 border-t border-gray-800 pt-2">Inputs</p>
              ...
              {INPUT_SOURCES.map(s => {
                ...
                return (
                  <div
                    ...
                    draggable
                    onDragStart={e => {
                      e.dataTransfer.setData("input_source", s.value);
                      e.dataTransfer.effectAllowed = "copy";
                      setIsDragging(true);
                    }}
                    onDragEnd={() => { setIsDragging(false); setDragOverStep(null); }}
                    onClick={() => addInputToSelectedStep(s.value)}
                    className="w-full flex items-center gap-2 p-2 rounded-xl border border-gray-800 hover:border-gray-600 bg-gray-900 hover:bg-gray-800 transition-all group cursor-grab active:cursor-grabbing select-none">
            ...
            {/* Output formats */}
            <p className="text-[9px] text-gray-600 uppercase tracking-wider font-semibold px-0.5 mb-1 border-t border-gray-800 pt-2">Outputs</p>
              ...
              {Object.entries(OUTPUT_FMT).map(([key, m]) => {
                ...
                return (
                  <div
                    ...
                    draggable
                    onDragStart={e => {
                      e.dataTransfer.setData("output_fmt", key);
                      e.dataTransfer.effectAllowed = "copy";
                      setIsDragging(true);
                    }}
                    onDragEnd={() => { setIsDragging(false); setDragOverStep(null); }}
                    className="w-full flex items-center gap-2 p-2 rounded-xl border border-gray-800 hover:border-gray-600 bg-gray-900 hover:bg-gray-800 transition-all group cursor-grab active:cursor-grabbing select-none">
                    <div className={cn("w-6 h-6 rounded-lg flex items-center justify-center shrink-0 border", m.border, m.bg)}>
                      <Icon className={cn("w-3 h-3", m.text)} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] font-semibold text-gray-200 truncate">{m.label}</p>
                    </div>
                    <span className="text-gray-700 text-[8px] shrink-0">⠿</span>
                  </div>
                );
        ...
        {/* Center: canvas — also a drop target for agent_cls */}
        <div
          ...
          onDragOver={e => { if (e.dataTransfer.types.includes("agent_cls") || e.dataTransfer.types.includes("input_source") || e.dataTransfer.types.includes("output_fmt")) e.preventDefault(); }}
          onDrop={e => {
            const cls = e.dataTransfer.getData("agent_cls");
            if (cls) { e.preventDefault(); addStep(cls); setIsDragging(false); }
          }}
        >
          {!showCanvas ? (
            ...
            <div className="flex items-start flex-nowrap gap-0">
              {pipelineForm.steps.map((step, i) => (
                <StepCard
                  ...
                  isDragOver={dragOverStep === i}
                  isAnyDragActive={isDragging}
                  onDragOver={e => { e.preventDefault(); setDragOverStep(i); }}
                  onDragLeave={() => setDragOverStep(null)}
                  onDrop={e => {
                    e.preventDefault();
                    e.stopPropagation();
                    const src = e.dataTransfer.getData("input_source") as SourceValue;
                    const fmt = e.dataTransfer.getData("output_fmt");
                    if (src) addInputToStep(i, src);
                    if (fmt) setOutputFmtOnStep(i, fmt);
